...
class Timeline extends React.Component {
	...
	outputHeader( dates ) {
		return dates.map( ( date, index ) => {
			return <div className="timeline-grid__header" key={ `header-${ index }` }>
	...
	outputTimeline( entries, gridStartDate ) {
		return entries.map( ( entry, index ) => {
			...
			const durationBetween = moment.duration( endDate.diff( startDate ) ).asDays() + 1;
			// Get the start point.
			const startPoint = moment.duration( startDate.diff( gridStartDate ) ).asDays();
	...
	render() {
		...
		return <div className="timeline-grid__container">
			<div className="timeline-grid__row">
				{ this.outputHeader( datesBetween ) }
			</div>
			<div className="timeline-grid__content" style={{ height: `${ 40 * data.length }px` }}>
